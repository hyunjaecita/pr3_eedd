...
class AVL { //arbol avl es un arbol balanceado
    ...
    bool inserta(NodoA<T>* &c, T &dato);
    void rotDecha(NodoA<T>* &p);
    void rotIzqda(NodoA<T>* &p);
    NodoA<T> *buscaClave(T &dato, NodoA<T>* &p);
 ...
 */
template<typename T>
bool AVL<T>::inserta(NodoA<T>* &c, T &dato){
    NodoA<T> *p = c;
    int deltaH = 0;
    if (!p){
        p = new NodoA<T>(dato);
 ...
 */
template <typename T>
NodoA<T> *AVL<T>::buscaClave(T &dato, NodoA<T>* &p) {
    if (!p)
        return 0;
    else if (dato < p->dato)
...
T* AVL<T>::buscaRec(T &dato){
    NodoA<T> *p = buscaClave (dato, raiz);
    /*if (p) {
        return (p->dato);
    }*/
    return p;
